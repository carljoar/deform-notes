...
\]
By the smoothness of $\exp$, the tangent space at $p'$ on the orbit $G.p$ can be constructed as the space $\{X_M\in T_{p'}M : X\in\mf g\}$.\todo{What is the relation to $p'$? Why different infinitesimal generators?} The group action $\theta:G\times M\to M$ induces an infinitesimal action. Fix $p\in M$ and consider $\theta^p:G\to M$. The corresponding tangent map $\theta^p_*:TG\to T_pM$ is called the infinitesimal action of the Lie group $G$ on $M$. The map is smooth in $p$, so it induces an action $TG\to TM$. We will denote this action $\xi.X$. 
...
Lie algebra structure, which is defined as follows:  For an arbitrary $g\in G$, define $L_g: G\to G$ by $L_gh=gh$. If $\xi$ is a vector field on $G$, we say that $\xi$ is left invariant if for any $h\in G$,
\[
T_hL_g(\xi_h) = \xi_{gh}\quad \mathrm{or}\quad (L_g)_*\xi=\xi.
\]
Every left invariant vector field on a Lie group $G$ is complete. That is, for each left invariant tangent vector $\xi\in TG_e$ there is a flow $a\subset G$ with $(a^e)'(0)=\xi$.\todo{Is this why we may define the infinitesimal action via $\xi$?}
...
The infinitesimal action map $\xi.p$, which is the tangent map $\theta^p_*$, is linear in the vector field. Therefore, it admits the definition of a \textit{momentum map} $J$ from the cotangent bundle $T^*M$ into the dual of the Lie algebra $\mf g$ through the relation
\[
\langle J(p,m),\xi\rangle = \langle m,\xi.p\rangle.
\]
...
\begin{proof}
	Given that the group of diffeomorphisms on $M$ has left action \textit{push forward} (and right action pullback), the infinitesimal action of a vector field $\xi$ on $X\in TM$ is
	\[
	\xi.X = \left.\frac{d}{dt}\right|_{t=0} \varphi^p(t)_*X = -\lie_\xi X.
	\]
	Here, $\xi$ generates the flow $\varphi :\R\times G \to G$.
\end{proof}
