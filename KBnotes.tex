...
\]
By the smoothness of $\exp$, the tangent space at $p'$ on the orbit $G.p$ can be constructed as the space $\{X_M\in T_{p'}M : X\in\mf g\}$. [Check $p'$.] The group action $\theta:G\times M\to M$ induces an infinitesimal action. Fix $p\in M$ and consider $\theta^p:G\to M$. The corresponding tangent map $\theta^p_*:TG\to T_pM$ is called the infinitesimal action of the Lie group $G$ on $M$. The map is smooth in $p$, so we can think of $\theta_*:TG\to TM$.
...
\]
Every left invariant vector field on a Lie group $G$ is complete. That is, for each left invariant tangent vector $\xi\in TG_e$ there is a flow $a\subset G$ with $(a^e)'(0)=\xi$.
...
\[
\mathrm{Ver}_g=\{L_{e*}\,g\in T_{g}G : (L_{e*}\,g).p=0\}.
\]
