...
\begin{proof}
	...
	(g\ \mathrm{symm.}\implies \xi_k=g_{jk}\xi^j=g_{kj}\xi^j\implies \frac{1}{2}(\lie_\xi g)_{ik}=\frac{1}{2}(\nabla_i\xi_k+\nabla_k\xi_i)=\nabla_k\xi_i)\\
	=\int_M\tr(\alpha \nabla^g\xi)\,\Vol_g=-\int_M \nabla^g_i\alpha^{ik}\xi_k\,\Vol_g=((\delta_g \alpha)^\flat,\xi)_g.
	\end{multline*}
	%
	Here, the flat map is $\omega \mapsto\omega^\flat= g \omega$. We conclude that
	\[
...
\todo[inline]{Check if $$\xi.S=-\lie_\xi(\varphi_*S)$$}
