...
\]
By the smoothness of $\exp$, the tangent space at $p'$ on the orbit $G.p$ can be constructed as the space $\{X_M\in T_{p'}M : X\in\mf g\}$.\todo{What is the relation to $p'$?} The group action $\theta:G\times M\to M$ induces an infinitesimal action. Fix $p\in M$ and consider $\theta^p:G\to M$. The corresponding tangent map $\theta^p_*:TG\to T_pM$ is called the infinitesimal action of the Lie group $G$ on $M$. We will denote this action $\xi.p$.\todo{What is the relation to $\xi$?} The map is smooth in $p$, so we can think of $\theta_*:TG\to TM$. 
