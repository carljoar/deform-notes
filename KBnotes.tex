...
\]
A \textit{Lie group} is a smooth manifold $G$ such that its group operations are smooth maps. A \textit{Lie algebra} is a vector space $\mf g$, together with a bilinear skewsymmetric operation $\mf g \times \mf g \to\mf g$  which satisfies the Jacobi identity. 
...
\[
  (\xi,\eta)_g=\langle A_g \xi,\eta\rangle=\langle A_g \eta,\xi\rangle=(\eta,\xi)_g.
\]
...
Since $G$ acts on $M$ from the left, the action map induces a Riemannian structure
on $G.p$. To understand this, we need the notion of horizontal vectors on $G$.
...
The group of volume-preserving diffeomorphisms of the manifold $M$ consists of all diffeomorphisms of $M$ preserving the volume form $\mu$,
\[
S\Diff(M)=\{\phi\in\Diff(M): \phi^*\mu=\mu\}.
\]
